...
export async function notifyVoluntariosSolicitacaoDisponivel(
  beneficiadoNome: string,
  necessidade: string
) {
  try {
    ...
    const htmlEmail = `
      <div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #eaeaec; border-radius: 8px; overflow: hidden;">
        <div style="background-color: #f59e0b; padding: 30px; text-align: center;">
          <h1 style="color: #fff; margin: 0; font-size: 24px;">SOLICITAÇÃO DISPONÍVEL NOVAMENTE</h1>
        </div>

        <div style="padding: 30px; background-color: #fcfcfc;">
          <p style="font-size: 16px; line-height: 1.5;">Olá, Amigo(a)! Uma solicitação de ajuda ficou sem responsável e está disponível novamente para ser assumida.</p>

          <div style="background-color: #fff; border-left: 4px solid #f59e0b; padding: 20px; margin: 25px 0; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
            <h2 style="margin-top: 0; color: #000;">${beneficiadoNome}</h2>
            <p style="margin: 5px 0; color: #666;"><strong>Necessidade:</strong> ${necessidade}</p>
          </div>

          <p style="font-size: 16px;">Acesse a plataforma e assuma esta solicitação para que ${beneficiadoNome} receba o apoio necessário!</p>

          <div style="text-align: center; margin-top: 30px;">
            <a href="https://www.amigos-em-acao.online/auth/login" style="background-color: #f59e0b; color: #fff; padding: 14px 28px; text-decoration: none; border-radius: 30px; font-weight: bold; font-size: 16px; display: inline-block;">Ver Solicitações</a>
          </div>
        </div>
    ...
    await sendEmail({
      to: process.env.EMAIL_USER,
      bcc: emails,
      subject: `⚠️ Solicitação precisa de voluntário: ${beneficiadoNome}`,
      html: htmlEmail,
    })
